...
public class Server extends JFrame {
    ...
    public void execute() {
        try {
            ...
            for (int i = 1; i < clients.length; i++) { // i starts from 1 because 0 is the server itself
    ...
    private void notifyAllPlayersReady() {
        ...
        for (int i = 1; i < toClients.length; i++) {
            try {
    ...
    private void handleClient(int playerIndex) {
        try {
            DataInputStream fromClient = new DataInputStream(clients[playerIndex].getInputStream());
            while (true) {
